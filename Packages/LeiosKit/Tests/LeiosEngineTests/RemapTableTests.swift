...
final class RemapTableTests: XCTestCase {
    ...
    func testDragArming() {
        ...
        XCTAssertEqual(table.dragArmed(for: ModifierState(keyboardFlags: LeiosConstants.ModifierFlag.shift, buttons: [ButtonModifier(button: 4, level: 1)])), .threeFingerSwipe, "extra keyboard flags don't block a button-only precondition")
    }
}
