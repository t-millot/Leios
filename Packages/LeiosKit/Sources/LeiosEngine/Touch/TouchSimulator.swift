...
// Leios Engine — synthesized trackpad gestures: navigation swipe, smart zoom, rotation, magnification, dock swipe.
...
final class TouchSimulator {
    ...
    func postDockSwipe(delta dIn: Double, type: DockSwipeType, phase: IOHIDPhase, invertedFromDevice: Bool) {
        ...
        guard let hidEvent = IOHIDEventCreate(nil, IOHIDEventType(kIOHIDEventTypeDockSwipe), LeiosMachAbsoluteTime(), 0) else { return }
        ...
        if phase == .ended || phase == .cancelled {
            if let child = IOHIDEventCreate(nil, IOHIDEventType(kIOHIDEventTypeVelocity), LeiosMachAbsoluteTime(), 0) {
        ...
        if let cg {
            ...
            if !LeiosEventSetIOHIDEvent(cg, hidEvent) {
                ...
                LeiosIOHIDEventRelease(hidEvent)
                return
            }
        }
        LeiosIOHIDEventRelease(hidEvent)
