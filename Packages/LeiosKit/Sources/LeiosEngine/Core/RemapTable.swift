// RemapTable.swift
// Leios Engine — maps (modifier state, button, level, duration) → actions and drag gestures.
...
final class RemapTable {
    ...
    func update(buttons: [Int: ButtonMapping]) {
        ...
        for (button, mapping) in buttons {
            guard button >= 1 && button <= LeiosConstants.maxButton, !mapping.isEmpty else { continue }
