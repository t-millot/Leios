...
// Leios Engine — memoizes "which app is under the pointer", which is far too slow to ask twice.
...
final class AppUnderPointerCache {
    ...
    func startObserving() {
        ...
        Log.engine.info("App-under-pointer fast path (SkyLight): \(LeiosWindowAtPointIsAvailable() ? "available" : "UNAVAILABLE, falling back to the window list", privacy: .public)")
