...
// Leios Helper — startup ordering: signals → XPC → config → status menu → accessibility → Engine.
...
final class HelperAppDelegate: NSObject, NSApplicationDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        ...
        accessibility.onTrusted = { [weak self] in
            guard let self else { return }
            NSLog("Leios Helper: Accessibility granted, starting engine")
            self.engine.start()
        }
        accessibility.onRevoked = { [weak self] in
            guard let self else { return }
            NSLog("Leios Helper: Accessibility revoked, stopping engine")
            self.engine.stop()
        }
