...
// The engine's side lives in Packages/LeiosKit/Tests.
...
final class ButtonCaptureTests: XCTestCase {
    ...
    func testMiddleButtonIsTheFirstCapturableNumber() {
        XCTAssertEqual(LeiosConstants.minButton, 3, "The capture zone assumes the middle button is number 3")
    ...
    func testNumbersAboveTheSupportedRangeAreRejected() {
        let tooHigh = LeiosConstants.maxButton + 1
        XCTAssertEqual(ButtonCaptureResult.classify(button: LeiosConstants.maxButton, isAlreadyMapped: false),
                       .added(LeiosConstants.maxButton))
