...
// MousePilot — a drop zone that adds a mouse button by having the user press it.
...
struct ButtonCaptureZone: View {
    /// Called with the MousePilot button number (middle button is 3) of every press inside the zone.
    ...
    private var subtitle: String {
        ...
        case .outOfRange: return "MousePilot supports buttons \(MPConstants.minButton) through \(MPConstants.maxButton)."
...
private struct ButtonCaptureCatcher: NSViewRepresentable {
    ...
    final class CatcherView: NSView {
        ...
        // `buttonNumber` is zero-based (middle button is 2); MousePilot counts from one.
