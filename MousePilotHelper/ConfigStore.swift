// ConfigStore.swift
// MousePilot Helper — reads config.json and watches its directory for changes.
...
final class ConfigStore {
    ...
    var onChange: ((MousePilotConfig) -> Void)?

    private var source: DispatchSourceFileSystemObject?
    private var fd: Int32 = -1
    private var debounce: DispatchWorkItem?

    init(initial: MousePilotConfig) {
        config = initial
    }
    ...
    func startWatching() {
        ...
        guard fd >= 0 else {
            NSLog("MousePilot Helper: cannot watch config directory")
            return
        }
    ...
    func reload() -> Bool {
        do {
            ...
            NSLog("MousePilot Helper: config reload failed: \(error)")
            return false
        }
    }

    /// Writes a modified config (used by the status menu kill switches) and applies it.
    func update(_ mutate: (inout MousePilotConfig) -> Void) {
        var new = config
        mutate(&new)
        guard new != config else { return }
        config = new
        do { try ConfigFile.save(new) } catch { NSLog("MousePilot Helper: config save failed: \(error)") }
        onChange?(new)
    }
}
