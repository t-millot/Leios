...
// Leios — the HID vocabulary the Info tab reads devices with: usages, elements, device kinds.
...
enum PointerDeviceKind: Equatable {
    ...
    /// Mice first: they are the devices Leios exists for.
    var sortRank: Int {
        switch self {
        case .mouse: return 0
...
/// Which of Leios's features can reach a device. Derived from the device's own controls, not
...
enum EngineHandling: Equatable {
    ...
    static func forDevice(kind: PointerDeviceKind, elements: HIDElementSummary) -> EngineHandling {
        let buttons = elements.highestButton >= LeiosConstants.minButton
    ...
    var summary: String {
        ...
        case .notAffected:
            return "Nothing — Leios leaves this device alone"
        }
    }
    ...
    private static func buttonPhrase(upTo highest: Int) -> String {
        highest <= LeiosConstants.minButton
            ? MouseButtonNaming.name(LeiosConstants.minButton)
            : "Buttons \(LeiosConstants.minButton)–\(highest)"
    }
